...
import static com.android.aaptcompiler.android.LocaleDataTablesKt.MAX_SCRIPT_PARENT_DEPTH;
import static com.android.aaptcompiler.android.LocaleDataTablesKt.getLIKELY_SCRIPTS;
import static com.android.aaptcompiler.android.LocaleDataTablesKt.getREPRESENTATIVE_LOCALES;
import static com.android.aaptcompiler.android.LocaleDataTablesKt.getSCRIPT_CODES;
import static com.android.aaptcompiler.android.LocaleDataTablesKt.getSCRIPT_PARENTS;
...
public class LocaleData {
  ...
  private static int findParent(int packed_locale, final String script) {
    if (hasRegion(packed_locale)) {
      for (Map.Entry<String, Map<Integer, Integer>> entry : getSCRIPT_PARENTS().entrySet()) {
  ...
  static boolean isRepresentative(int language_and_region, final String script) {
    ...
    return (getREPRESENTATIVE_LOCALES().contains(packed_locale));
  }
  ...
  static int localeDataCompareRegions(
    ...
    } else if (rightIsSpecialSpanish
        && !leftIsSpecialSpanish
        && left != LATIN_AMERICAN_SPANISH) {
      right = LATIN_AMERICAN_SPANISH;
    }

    int[] request_ancestors = new int[MAX_SCRIPT_PARENT_DEPTH + 1];
  ...
  static void localeDataComputeScript(byte[] out, final byte[] language, final byte[] region) {
    ...
    Byte lookup_result = getLIKELY_SCRIPTS().get(lookup_key);
    if (lookup_result == null) {
      // We couldn't find the locale. Let's try without the region
      if (region[0] != '\0') {
        lookup_key = dropRegion(lookup_key);
        lookup_result = getLIKELY_SCRIPTS().get(lookup_key);
        if (lookup_result != null) {
          //          memcpy(out, SCRIPT_CODES[lookup_result.second], SCRIPT_LENGTH);
          System.arraycopy(getSCRIPT_CODES()[lookup_result], 0, out, 0, SCRIPT_LENGTH);
          return;
        }
      }
